...
macro_q <- macro_data %>%
  ...
  summarize(
    ...
    .groups = "drop" #dropping missing values
  ) %>%
...
patent_data <- patent_data %>%
  mutate(issue_date = ymd(issue_date))
sum(is.na(patent_data$issue_date))
glimpse(patent_data)

patent_data_clean <- patent_data %>%
  filter(issue_date > ymd("1968-12-31"),
         issue_date < ymd("1988-12-31")) %>%
  mutate(quarter_date = floor_date(issue_date, unit = "quarter")) 
glimpse(patent_data_clean)
...
full_df <- full_df %>%
  ...
  mutate(
    log_innovation = log(xi_real_sum),
    across(c(log_innovation, mps_t, FEDFUNDS, log_gdp, log_cpi, UNRATE),
           list(lag1 = ~lag(.x, 1),
                lag2 = ~lag(.x, 2),
                lag3 = ~lag(.x, 3),
                lag4 = ~lag(.x, 4)))
